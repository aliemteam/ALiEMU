...
if ( ! function_exists( 'aliemu_setup' ) ) :
	/**
	...
	function aliemu_setup() {
		...
		register_nav_menus( array(
			'menu-1' => esc_html__( 'Primary', 'aliemu' ),
		) );
		...
		add_theme_support( 'custom-background', apply_filters( 'aliemu_custom_background_args', array(
 ...
 */
function aliemu_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'aliemu_content_width', 640 );
}
add_action( 'after_setup_theme', 'aliemu_content_width', 0 );
 ...
 */
function aliemu_scripts() {
	if ( is_singular() && comments_open() && get_option( 'thread_comments' ) ) {
		wp_enqueue_script( 'comment-reply' );
	}
}
add_action( 'wp_enqueue_scripts', 'aliemu_scripts' );
